...
import { X, Dumbbell, FileBarChart, Leaf, Github, ExternalLink, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function ProjectModal({
  ...
  const navigateLightbox = (direction: 'prev' | 'next') => {
    ...
    if (direction === 'prev') {
      newIndex = currentIndex > 0 ? currentIndex - 1 : lightboxContent.screenshots.length - 1;
    } else {
      newIndex = currentIndex < lightboxContent.screenshots.length - 1 ? currentIndex + 1 : 0;
    }
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/90">
          ...
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-white/10 px-4 py-2 rounded-full">
            <span className="text-white text-sm font-medium">
              {lightboxContent.screenshots.indexOf(lightboxImage) + 1} of {lightboxContent.screenshots.length}
            </span>
          </div>
